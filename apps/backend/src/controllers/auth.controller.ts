...
export const verifyAndSetPassword = async (
    req: Request,
    res: Response
): Promise<void> => {
    try {
        ...
        if (!password) {
            res.status(200).json({
                message:
                    'Token valid. Silakan masukkan password baru melalui aplikasi/postman',
                token,
            });
            return;
        }

        const result = await verifyAndSetPasswordService(
            token as string,
            password
        );
